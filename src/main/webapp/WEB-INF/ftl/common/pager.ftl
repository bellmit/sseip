...
<div class="dataTables_paginate">
    <ul class="pagination">
        <li>
            <span>${page.pageNo}/${page.maxPageNo}页</span>
            <span>每页${page.pageSize}条</span>
        </li>
        <li>
            <span>共${page.totalRows}条</span>
        </li>
        ...
        -->
            <#if page.maxPageNo gte 3><#list  (page.pageNo-10>2)?then(page.pageNo-10,2)..(page.pageNo+10 lt page.maxPageNo)?then(page.pageNo+10,page.maxPageNo-1) as i>
                <li><#if page.pageNo=i><span>${i}</span><#else>
                    <a href="${baseUrl+'/'+i}${('?'+query)!''}">
                        <#switch i >
                            <#case page.pageNo-1>
                                上一页
                                <#break >
        ...
        -->
            <#if page.maxPageNo gte 2>
                <li>
                    <#if !page.lastPage><a href="${baseUrl+'/'+page.maxPageNo}${('?'+query)!''}">尾页</a><#else>
                        <span>尾页</span></#if>
                </li>
            </#if>
        </#if>
