...
//        ps = conn.prepareStatement("select count(1) from 查询");
        ps = conn.prepareStatement("select length(mzbeizhu),ID from 查询");
...
//        LinkedList<Long> ll = new LinkedList<>();
        ...
        while (resultSet.next()) {
            ...
            if (l > 20000) {
                System.out.println(String.format("%d\t%d", resultSet.getLong(1), resultSet.getLong(2)));
            }
            al.add(l);
