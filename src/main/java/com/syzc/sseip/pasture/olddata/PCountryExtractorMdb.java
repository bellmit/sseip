...
public class PCountryExtractorMdb {
...
//                s.add(emap.get(i));
    ...
    public static Long t(ResultSet c) throws SQLException {
        Matcher am;

        if (v(c.getString("Address")) != null) {
            return v(c.getString("Address"));
        } else if (v(c.getString("kfuseraddr")) != null) {
            return v(c.getString("kfuseraddr"));
        } else {
            if (c.getString("LtRecord") != null) {

                am = pContactCountry.matcher(c.getString("LtRecord"));
                if (am.find()) {
