...
public class CountryExtractorPracticeA {
...
//                s.add(emap.get(i));
    ...
    private static Long t(CSVRecord c) {
        Matcher am;
...
/*
            if (v(c.get("Address")).size() > 0) {
                if (v(c.get("Address")) != null) {
                    if (cty != null && v(cty).size() == 0) {
                        System.out.println("" + c.getRecordNumber() + v(c.get("Address")) + "," + v(cty) + "," + cty);

//                        if (v(c.get("Address")).size() == 1) {
